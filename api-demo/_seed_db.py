...
from app.core.models.self_healing import Runbook
...
async def seed():
    async with session_factory() as sess:
            ...
            for sli_name, target, current in [
                ...
                bps = [
                    HealthSnapshot(
                        ...
                        sli_name=sli_name,
                        slo_target=target,
                        current_value=current if current > target else current,
                        burn_rate=0.0,
        ...
        inc1 = Incident(
            id=uuid.uuid4(),
            title="API Gateway — P50 Latency Breach",
            severity="SEV-3",
            status="resolved",
            service="api-gateway",
        ...
        inc2 = Incident(
            id=uuid.uuid4(),
            title="Payment Service — 5xx Error Spike",
            severity="SEV-2",
        ...
        sess.add(
            RcaReport(
                ...
                timeline=[
                    {"time": str(now - timedelta(hours=6)), "event": "deploy v2.1.5"},
                    {
                        "time": str(now - timedelta(hours=6, minutes=10)),
                        "event": "p50 latency breach alert",
                    },
                ],
