...
class MttrBreakdown:
    ...
    def __post_init__(self):
        self.total_min = (
            self.mttd_min + self.mtti_min + self.mttk_min + self.mtta_min + self.mttr_min
        )
...
SCENARIOS: dict[str, dict[str, Any]] = {
    "deploy": {
        ...
        "changes": [
            {"type": "deploy", "service": "api-gateway", "version": "v2.1.5", "rollback": "v2.1.4"}
        ],
        ...
        "runbook_steps": [
            ...
            {
                "order": 2,
                "action": "Check p50/p99 latency graphs",
                "command": "datadog dashboard latency-spike",
            },
            {
                "order": 3,
        ...
        "self_heal_action": {
            "type": "rollback",
            "config": {"target_version": "v2.1.4", "service": "api-gateway"},
        },
    ...
    "resource": {
        ...
        "changes": [
            {"type": "config", "service": "payment-service", "change": "hpa_max_replicas 3→10"}
        ],
        "runbook_name": "CPU Saturation Response",
        "runbook_steps": [
            {
                "order": 1,
                "action": "Check current CPU and HPA status",
                "command": "kubectl top pods -l app=payment",
            },
            {
                "order": 2,
                "action": "Increase HPA max replicas",
                "command": 'kubectl patch hpa payment-service -p \'{"spec":{"maxReplicas":10}}\'',
            },
            {
                "order": 3,
                "action": "Check queue backlog depth",
                "command": "kubectl exec payment-worker -- rabbitmqctl list_queues",
            },
            {"order": 4, "action": "Monitor recovery", "command": "check cpu < 80% for 5min"},
        ],
        "self_heal_action": {
            "type": "scale",
            "config": {"replicas": 10, "service": "payment-service"},
        },
        "monitor_query": "avg(last_5m):avg:system.cpu.user{service:payment-service} > 85",
        "slo_target": 99.5,
        "mttr": MttrBreakdown(
            "resource", mttd_min=5, mtti_min=10, mttk_min=5, mtta_min=3, mttr_min=10
        ),
    },
    "latency": {
        ...
        "changes": [
            {
                "type": "schema",
                "service": "user-service",
                "change": "index dropped during migration v3.2.0",
            }
        ],
        ...
        "runbook_steps": [
            ...
            },
            {
                ...
                "command": "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE now() - query_start > interval '30s'",
            },
            {
                "order": 3,
                "action": "Recreate missing index",
                "command": "CREATE INDEX CONCURRENTLY idx_users_org ON users(organization_id)",
            },
            {"order": 4, "action": "Verify recovery", "command": "check p99 < 200ms for 5min"},
        ],
        "self_heal_action": {
            "type": "script",
            "config": {"script": "pg_recreate_index_users_org.sh", "service": "user-service"},
        },
        "monitor_query": "avg(last_5m):avg:system.cpu.user{service:user-service} > 85",
        "slo_target": 99.0,
        "mttr": MttrBreakdown(
            "latency", mttd_min=3, mtti_min=15, mttk_min=5, mtta_min=2, mttr_min=8
        ),
    },
    "dependency": {
        ...
        "changes": [
            {"type": "config", "service": "redis", "change": "maxmemory-policy unset → allkeys-lru"}
        ],
        "runbook_name": "Redis Degraded/Failover Response",
        "runbook_steps": [
            {
                "order": 1,
                "action": "Check Redis node roles",
                "command": "redis-cli -h redis-cluster info replication",
            },
            {
                "order": 2,
                "action": "Force failover to replica",
                "command": "redis-cli -h redis-replica cluster failover",
            },
            {
                "order": 3,
                "action": "Set eviction policy on primary",
                "command": "redis-cli config set maxmemory-policy allkeys-lru",
            },
            {
                "order": 4,
                "action": "Verify session recovery",
                "command": "check /api/auth endpoint",
            },
        ],
        "self_heal_action": {
            "type": "script",
            "config": {"script": "redis_failover.sh", "service": "redis"},
        },
        "monitor_query": "avg(last_5m):avg:system.cpu.user{service:api-gateway} > 85",
        "slo_target": 99.9,
        "mttr": MttrBreakdown(
            "dependency", mttd_min=1, mtti_min=12, mttk_min=4, mtta_min=3, mttr_min=15
        ),
    },
    "data_corruption": {
        ...
        "changes": [
            {
                "type": "migration",
                "service": "order-service",
                "change": "migration v4.0 adding NOT NULL without backfill",
            }
        ],
        ...
        "runbook_steps": [
            {
                "order": 1,
                "action": "Run backfill query",
                "command": "UPDATE orders SET discount_pct=0 WHERE discount_pct IS NULL",
            },
            {
                "order": 2,
                "action": "Drop NOT NULL constraint",
                "command": "ALTER TABLE orders ALTER COLUMN discount_pct DROP NOT NULL",
            },
            {
                "order": 3,
                "action": "Fix migration script",
                "command": "add backfill step BEFORE constraint",
            },
            {
                "order": 4,
                "action": "Verify data integrity",
                "command": "SELECT count(*) FROM orders WHERE discount_pct IS NULL",
            },
        ],
        "self_heal_action": {
            "type": "script",
            "config": {"script": "rollback_migration_v4.sh", "service": "order-service"},
        },
        "monitor_query": "avg(last_5m):avg:system.cpu.user{service:order-service} > 85",
        "slo_target": 99.5,
        "mttr": MttrBreakdown(
            "data_corruption", mttd_min=4, mtti_min=20, mttk_min=8, mtta_min=5, mttr_min=25
        ),
    },
}
...
class DdClient:
    ...
    def send_metric(
        self, name: str, value: float, mtype: str = "gauge", tags: list[str] | None = None
    ) -> bool:
        body = {
            "series": [
                {
    ...
    def send_log(
        self, msg: str, status: str = "info", svc: str = "", tags: list[str] | None = None
    ) -> bool:
        body = [
            {
                "ddsource": "sre-engine",
    ...
    def send_event(
        self, title: str, text: str, alert_type: str = "info", tags: list[str] | None = None
    ) -> bool:
        body = {
            "title": title,
            "text": text,
    ...
    def create_monitor(
        self, name: str, query: str, msg: str, tags: list[str] | None = None
    ) -> dict[str, Any] | None:
        if not self.app_key:
            return None
        body = {
    ...
    def create_incident(
        self, title: str, severity: str = "SEV-3", svc: str = ""
    ) -> dict[str, Any] | None:
        if not self.app_key:
            return None
        body = {
        ...
        if severity in ("SEV-1", "SEV-2"):
            ...
            body["data"]["attributes"]["customer_impact_scope"] = (
                "60% of authenticated users affected"
            )
...
def db_ensure_tables():
    ...
    p = subprocess.run(
        [
            sys.executable,
            "-c",
            "from app.core.db import engine, Base; Base.metadata.create_all(bind=engine); print('tables ok')",
        ],
        cwd="/opt/data/workspace/observai/backend",
        capture_output=True,
        text=True,
        timeout=10,
    )
...
def run_scenario(name: str, dd: DdClient) -> dict[str, Any]:
    ...
    metric_templates = {
        "deploy": [
            (
                f"{svc}.latency_ms",
                lambda i: round(
                    120 + (1100 * (1 - math.exp(-(i - 2) * 0.5)))
                    if i >= 2
                    else 120 + random.gauss(10, 5),
                    1,
                ),
            ),
        ...
        "latency": [
            (
                f"{svc}.latency_ms",
                lambda i: round(min(50 * (1 + i * 0.8) + random.gauss(0, 20), 8000), 1),
            ),
    ...
    dd.send_event(
        f"[{cfg['severity']}] {cfg['title'][:80]}", event_texts.get(name, ""), "error", tags
    )
    results["events_sent"] = 1

    # ── Phase 2: Create monitor in Datadog ──────────────────────
    print("  [2/6] Creating monitor + incident...")
    ...
    print("  [3/6] Creating local DB records...")
    ...
    db_insert(
        "incidents",
        {
            ...
            "started_at": _time_ago(
                int(mttr.mttd_min + mttr.mtti_min + mttr.mttk_min + mttr.mtta_min + 5)
            ),
    ...
    db_insert(
        "rca_reports",
        {
            ...
            "metrics_snapshot": {
                "service": svc,
                "scenario": name,
                "mttd_min": mttr.mttd_min,
                "mtti_min": mttr.mtti_min,
    ...
    # ── Phase 5: SLO ────────────────────────────────────────────
    print("  [5/6] Creating SLO in Datadog...")
                ...
                "thresholds": [
                    {
                        ...
                        "timeframe": "30d",
                        "warning": min(cfg["slo_target"] + 0.5, 99.95),
                    }
                ],
                "monitor_ids": [mid],
                "tags": tags,
...
def main():
    ...
    if args.list:
        ...
        for n, c in SCENARIOS.items():
            m = c["mttr"]
            print(
                f"{n:25s} {c['severity']:10s} {c['service']:20s} {m.total_min:<10.0f} {c['title'][:55]}"
            )
        return
    ...
    scenarios_to_run = (
        list(SCENARIOS.keys()) if (args.scenario == "all" or args.all) else [args.scenario]
    )
    ...
    print("  Run assessment: POST /api/v1/maturity/assess")
    print(f"{'=' * 55}")
