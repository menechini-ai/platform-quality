    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div
        ...
        className={clsx(
          ...
          "bg-surface-800 border border-slate-700/50 rounded-t-xl md:rounded-xl",
          "shadow-2xl animate-slide-up-md",
          className,
        )}
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50 sticky top-0 bg-surface-800/95 backdrop-blur z-10 rounded-t-xl md:rounded-t-xl">
      ...
      <div className={clsx("flex items-center gap-2 font-mono text-sm", isEmpty && "text-slate-500")}>
        ...
        {copyable && typeof value === "string" && !isEmpty && (
          <button
            ...
            className="p-0.5 rounded text-slate-500 hover:text-white hover:bg-slate-700/50 opacity-0 group-hover:opacity-100 transition-opacity"
            aria-label="Copy"
          >
...
export function PeekSection({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={clsx("peek-section space-y-4", className)}>
      <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/50 pb-2">
        {title}
      </h3>
