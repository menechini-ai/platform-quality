    ...
    return [
        NormalizedIncident(
            ...
            annotations={
                ...
                "user_count": str(issue.user_count),
            },
            started_at=datetime.fromisoformat(issue.first_seen.replace("Z", "+00:00")),
...
def normalize_cloudwatch(payload: CloudWatchPayload) -> list[NormalizedIncident]:
    ...
    try:
        ...
        logger.warning("Failed to parse CloudWatch message: %s", e)
    ...
    for record in payload.records:
        ...
        incidents.append(
            NormalizedIncident(
                ...
                started_at=(
                    datetime.fromtimestamp(int(record.time) / 1e9, UTC)
...
def detect_source(payload: dict) -> str:
    ...
    if "receiver" in payload and "alerts" in payload and "group_labels" in payload:
        if "generator_url" in payload.get("alerts", [{}])[0]:
            return "alertmanager"
        return "grafana"
...
async def receive_incident(
    ...
    logger.info("Received incident from %s", source)
    ...
    logger.info("Created %d incidents from %s", len(created), source)
...
async def test_webhook(request: Request):
    ...
    return {
        ...
        "sample": normalized[0].__dict__ if normalized else None,
    }
